...
# Variabele om bij te houden of het startscherm nog moet worden getoond (TG)
start_screen = True  # In het begin moet het startscherm worden getoond (TG)

running = True
while running:
    win.fill(BLACK)
    # Startscherm tonen als start_screen waar is
    if start_screen:
        # Teken het startscherm (lijkt op het game over scherm) (TG)
        rect = pygame.Rect((50, 140, WIDTH-100, HEIGHT-350))
        pygame.draw.rect(win, BLACK, rect)
        pygame.draw.rect(win, BLUE, rect, 2)  # Blauwe rand voor startscherm (TG)
        
        # Tekst voor het startscherm (TG)
        title = font2.render('Tetris', True, WHITE)
        start_msg = font2.render('Druk op R om te beginnen', True, BLUE)
        quit_msg = font2.render('Druk op ESC om het spel te sluiten', True, RED)
        
        # De tekst op het scherm zetten (TG)
        win.blit(title, (rect.centerx-title.get_width()/2, rect.y + 20))
        win.blit(start_msg, (rect.centerx-start_msg.get_width()/2, rect.y + 80))
        win.blit(quit_msg, (rect.centerx-quit_msg.get_width()/2, rect.y + 110))
        
        # HUD ook tekenen zodat het er compleet uitziet (TG)
        pygame.draw.rect(win, BLUE, (0, HEIGHT-120, WIDTH, 120))
        scoreimg = font.render('0', True, WHITE)
        levelimg = font2.render('Level : 1', True, WHITE)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            # Als we in het startscherm zijn (TG)
            if start_screen:
                if event.key == pygame.K_r:  # Met r begin je het spel (TG)
                    start_screen = False  # Startscherm uitzetten (TG)
                    # Spel opnieuw beginnen voor de zekerheid (TG)
                    tetris.__init__(ROWS, COLS)
                if event.key == pygame.K_ESCAPE:  # ESC om af te sluiten (TG)
                    running = False
            else:
